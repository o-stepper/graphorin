 ...
 * coverage therefore runs without any LLM dependency, while the
 * shape of every reply still mimics a real exchange so the
 * consolidator's standard phase has plausible facts to extract.
 */
...
export const STUB_ECHO_PREFIX = 'consolidator-echo: ';

/**
 * Build the deterministic two-sentence reply for the supplied
 * request. Exposed so tests can assert on the same string the
 * stream yields.
 */
 ...
 * {@link stubEchoReply} followed by a zero-usage `finish`.
 */
export function createStubProvider(options: { readonly modelId?: string } = {}): Provider {
  const modelId = options.modelId ?? 'background-consolidator-stub';
