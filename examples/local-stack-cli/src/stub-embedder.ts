 ...
 * an FNV-1a hash, so vector queries do not blow up but the embedder
 * can still run inside CI without an Ollama daemon. Never opens a
 * socket, never reads from disk, and emits the same vector for the
 * same input every time so snapshot tests are stable.
 */
...
export function createStubEmbedder(): EmbedderProvider {
  const id = `stub-local:8`;
  const cfg = createHash('sha256').update(`stub-local-embedder:${STUB_DIM}`).digest('hex');
  return {
    id(): string {
      return id;
    },
