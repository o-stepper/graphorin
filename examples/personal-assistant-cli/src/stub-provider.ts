...
export const STUB_ECHO_PREFIX = 'stub-echo: ';

/**
 * Build the deterministic echo reply for the supplied request. Exposed
 * so tests can assert on the same string the stream yields.
 */
 ...
 * `'stub-echo: ' + lastUserText` followed by a zero-usage `finish`.
 */
export function createStubProvider(options: { readonly modelId?: string } = {}): Provider {
  const modelId = options.modelId ?? 'stub-echo';
