...
This package holds **optional locale-specific question files** for the memory-retrieval QA benchmark runner (`@graphorin/benchmark-locomo`). The runner loads `questions.jsonl` from a locale directory when invoked with `--subset <locale>`.
...
2. Validate locally: `pnpm install && pnpm --filter @graphorin/benchmark-locomo build && node ./benchmarks/locomo/dist/runner.js --subset <locale>`.
3. Open a pull request with translation provenance (reviewers, source language).

Community-maintained packs ship **after** the v0.1.0 tag; the hooks and procedure above are stable from v0.1.0 onward.
