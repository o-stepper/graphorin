...
// V8 coverage instrumentation inflates the warm path ~5-8x, so the
// latency canary is meaningless there - skipped on the CI coverage leg
// (GRAPHORIN_COVERAGE=1) AND under a plain `vitest --coverage` run
// (auto-detected); the plain matrix legs still run it.
describe.skipIf(coverageInstrumented())('Phase 03b - verifyToken warm-cache latency', () => {
