...
Defined in: packages/store-sqlite/src/session-store.ts:415
...
| <a id="property-refs"></a> `refs?` | `readonly` | readonly \{ `column`: `string`; `table`: `string`; \}[] | Tables referencing the base rows - cleared BEFORE the base rows. | packages/store-sqlite/src/session-store.ts:425 |
| <a id="property-sessioncolumn"></a> `sessionColumn` | `readonly` | `"scope_session_id"` \| `"session_id"` | Column carrying the session id. | packages/store-sqlite/src/session-store.ts:419 |
| <a id="property-table"></a> `table` | `readonly` | `string` | Base table holding the session-scoped rows. | packages/store-sqlite/src/session-store.ts:417 |
| <a id="property-vec"></a> `vec?` | `readonly` | \{ `idColumn`: `string`; `prefix`: `string`; \} | Per-embedder vec0 sidecar family (name prefix + id column). | packages/store-sqlite/src/session-store.ts:423 |
| `vec.idColumn` | `readonly` | `string` | - | packages/store-sqlite/src/session-store.ts:423 |
| `vec.prefix` | `readonly` | `string` | - | packages/store-sqlite/src/session-store.ts:423 |
