...
| <a id="property-limits"></a> `limits?` | `readonly` | [`SchedulerLimits`](/api/@graphorin/triggers/interfaces/SchedulerLimits.md) | Opt-in scheduler harness (C4): interval floor + declaration cap enforced at `register(...)` time. Absent = no constraints (the pre-harness behaviour). Pass `{}` for the conservative defaults. | packages/triggers/src/index.ts:323 |
