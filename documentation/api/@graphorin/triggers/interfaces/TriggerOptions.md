...
| ------ | ------ | ------ | ------ | ------ |
| <a id="property-acknowledgelibmode"></a> `acknowledgeLibMode?` | `readonly` | `boolean` | Suppress the one-time per-process library-mode WARN. Library callers acknowledging that triggers fire only as long as the process lives pass `true` here. | packages/triggers/src/index.ts:49 |
