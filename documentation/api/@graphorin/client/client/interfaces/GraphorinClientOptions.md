...
| <a id="property-sessionid"></a> `sessionId?` | `readonly` | `string` | Session bound to the SSE fallback (IP-3): substituted into the `:sessionId` slot of `sseSessionPath`. Required to connect over SSE - the old client sent the literal template and could never receive an event. | packages/client/src/graphorin-client.ts:120 |
