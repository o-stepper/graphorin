...
| <a id="property-lasteventid"></a> `lastEventId?` | `readonly` | `string` | Resume cursor sent as the `Last-Event-ID` header on (re)connect. The server replays only events AFTER it from the buffer - without it every SSE reconnect replays the entire buffered history. Consumed by the SSE transport; ignored by WS (whose resubscribe carries the cursor in the RPC). | packages/client/src/transport/types.ts:39 |
