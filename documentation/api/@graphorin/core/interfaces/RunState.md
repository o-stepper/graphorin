...
| <a id="property-pendingsubruns"></a> `pendingSubRuns?` | `public` | `PendingSubRun`[] | Sub-agent runs parked on this (parent) run because the child suspended with `awaiting_approval`. Each entry snapshots the suspended child state; the child's pending approvals are mirrored onto this run's `pendingApprovals` with `subRunToolCallId` set to the entry's `toolCallId`. Absent until a child parks. | packages/core/src/types/run.ts:157 |
