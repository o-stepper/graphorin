...
| <a id="property-toolname"></a> `toolName?` | `readonly` | `string` | Convenience duplicate of the executing tool's name. Correlation within a tool lifecycle is by `toolCallId`; this field spares direct subscribers a stateful join back to the `tool.call.start` that carried the name. Optional for wire compatibility; the agent runtime always fills it. | packages/core/src/types/agent-event.ts:153 |
