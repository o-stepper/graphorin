...
| ------ | ------ | ------ | ------ | ------ |
| <a id="property-mode"></a> `mode?` | `readonly` | `"ask"` \| `"defer"` | E1: which permission verdict parked the approval (`'ask'` | `'defer'`); absent for plain `needsApproval` gates. Mirrors `ToolApproval.mode` so a subscriber can route deferred approvals without re-reading the run state. | packages/core/src/types/agent-event.ts:222 |
