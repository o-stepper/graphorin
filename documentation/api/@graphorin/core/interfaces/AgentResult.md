...
| <a id="property-verdicts"></a> `verdicts?` | `readonly` | [`RunVerdicts`](/api/@graphorin/core/type-aliases/RunVerdicts.md) | B3 (item 15): the run's per-turn security verdicts (mirrors `state.verdicts`). Surfaced directly so callers can apply them at the `Session.push` boundary without digging into the state. | [packages/core/src/types/agent-event.ts:577](https://github.com/o-stepper/graphorin/blob/main/packages/core/src/types/agent-event.ts#L577) |
