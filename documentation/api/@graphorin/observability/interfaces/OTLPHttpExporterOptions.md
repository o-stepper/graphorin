...
| <a id="property-timeoutms"></a> `timeoutMs?` | `readonly` | `number` | Optional timeout (ms). Defaults to 10000. | packages/observability/src/exporters/otlp-http.ts:41 |
| <a id="property-url"></a> `url` | `readonly` | `string` | OTLP collector URL (e.g. `http://localhost:4318/v1/traces`). | packages/observability/src/exporters/otlp-http.ts:33 |
