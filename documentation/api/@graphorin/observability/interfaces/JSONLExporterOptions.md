...
| <a id="property-maxopenhandles"></a> `maxOpenHandles?` | `readonly` | `number` | Upper bound on simultaneously-open file handles (RP-20). The pool keys by `(session, UTC-month)`, so a long-living server would otherwise hold one fd per pair forever; the least-recently-used handle is closed once the cap is reached. Defaults to `64`. | packages/observability/src/exporters/jsonl.ts:56 |
