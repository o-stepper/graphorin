...
| ------ | ------ | ------ | ------ | ------ |
| <a id="property-description"></a> `description?` | `readonly` | `string` | Optional description shown to the model alongside the schema. | packages/agent/src/types.ts:67 |
...
| <a id="property-schema"></a> `schema?` | `readonly` | \{ `parse`: `TOutput`; \} | Local validator (Zod-compatible `{ parse }`) applied to the final model output on the completed path (AG-3). A parse failure fails the run with `output-validation-failed` - never a silent cast. | packages/agent/src/types.ts:65 |
