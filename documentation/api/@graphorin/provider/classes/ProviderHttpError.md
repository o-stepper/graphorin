...
Defined in: packages/provider/src/errors/errors.ts:274
...
| <a id="property-retryafterms"></a> `retryAfterMs?` | `readonly` | `number` | `undefined` | Server-provided backoff hint in milliseconds, stamped at construction when the captured `retry-after` header carries a numeric seconds value. HTTP-date `Retry-After` values are left to `readRetryAfterMs` (resolved against the clock at retry time, not at construction). | - | - | packages/provider/src/errors/errors.ts:272 |
