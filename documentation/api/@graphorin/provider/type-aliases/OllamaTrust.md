[**Graphorin API reference v0.13.11**](../../../index.md)

***

[Graphorin API reference](/api/index.md) / [@graphorin/provider](/api/@graphorin/provider/index.md) / [](/api/@graphorin/provider/README.md) / OllamaTrust

# ~~Type Alias: OllamaTrust~~

```ts
type OllamaTrust = LocalProviderTrust;
```

Defined in: [packages/core/dist/contracts/local-provider-trust.d.ts](https://github.com/o-stepper/graphorin/blob/main/packages/core/dist/contracts/local-provider-trust.d.ts)

Legacy alias preserved for downstream packages that still import the
Ollama-specific name. Removed in v0.2 per the legacy-sugar
deprecation policy.

## Deprecated

Use [LocalProviderTrust](/api/@graphorin/provider/type-aliases/LocalProviderTrust.md) instead. Removed in v0.2.
