...
| <a id="property-verdict"></a> `verdict?` | `readonly` | [`RunTurnVerdict`](/api/@graphorin/core/interfaces/RunTurnVerdict.md) | **`Stable`** B3 (item 15): the turn's persisted security verdict, when the run loop stamped one (`SessionMessagePushOptions.verdict`). Read by the memory ingest gate to exclude guardrail-blocked turns from extraction. Additive to the tuple. | packages/memory/src/internal/storage-adapter.ts:243 |
