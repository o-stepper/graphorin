...
| Class | Description |
| ------ | ------ |
| [EmbedderMigrationAbortedError](/api/@graphorin/memory/errors/classes/EmbedderMigrationAbortedError.md) | Raised by `migrateEmbedder(...)` when the runner is interrupted via `AbortSignal`. The surrounding `for-await` loop receives this error so the operator can re-run the migration. MST-12: there is no persisted cursor today, so a re-run restarts from the beginning. |
