...
| <a id="property-maxtrackedips"></a> `maxTrackedIps?` | `readonly` | `number` | Cap on distinct IPs tracked in the failure/lockout maps. Default 10 000 - overflow sweeps expired lockouts, then evicts the oldest entries. | packages/security/src/auth/verify.ts:117 |
