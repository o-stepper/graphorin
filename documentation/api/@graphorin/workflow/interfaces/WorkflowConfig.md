...
| <a id="property-maxconcurrenttasks"></a> `maxConcurrentTasks?` | `readonly` | `number` | Cap on tasks executing concurrently within one step (D1 / workflow-10). Planned tasks beyond the cap queue and start as slots free up; `Dispatch` fan-outs are bounded the same way. Absent ⇒ unbounded (the pre-D1 behaviour). | packages/workflow/src/types.ts:335 |
...
| <a id="property-maxtotalsteps"></a> `maxTotalSteps?` | `readonly` | `number` | W-122: opt-in LIFETIME quota over the cumulative step number across every invocation of the thread. Default: undefined (no lifetime cap). Fails with the same `max-steps-exceeded` code but a distinct message. | packages/workflow/src/types.ts:308 |
