...
In-memory [CheckpointStore](/api/@graphorin/workflow/interfaces/CheckpointStore.md) adapter. Useful in tests, REPL
...
| Class | Description |
| ------ | ------ |
| [InMemoryCheckpointStore](/api/@graphorin/workflow/checkpoint-store-memory/classes/InMemoryCheckpointStore.md) | Pure in-memory [CheckpointStore](/api/@graphorin/workflow/interfaces/CheckpointStore.md) implementation. Thread-safe within a single Node.js event loop because every mutation is synchronous; concurrent runs that share the same instance will see a consistent view. |
