...
| <a id="property-budget"></a> `budget?` | `readonly` | `Pick`\&lt;[`RunBudget`](/api/@graphorin/agent/interfaces/RunBudget.md), `"maxCostUsd"` \| `"maxTokens"`\&gt; | Per-fire run budget (C5). `onExceed` is pinned to `'stop'`. | packages/proactive/src/cron-task.ts:143 |
