...
| ------ | ------ | ------ | ------ | ------ | ------ |
| <a id="property-batchsize"></a> `batchSize?` | `readonly` | `number` | Rows per re-embed batch. Default `512`. | - | packages/cli/src/commands/memory.ts:131 |
...
| <a id="property-reclaim"></a> `reclaim?` | `readonly` | `boolean` | After a committed migration, drop the RETIRED embedders' vector sidecar tables and run `PRAGMA incremental_vacuum` (space reclaim). Default `false`. | - | packages/cli/src/commands/memory.ts:137 |
