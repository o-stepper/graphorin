...
| <a id="property-passphrasefrom"></a> `passphraseFrom` | `readonly` | `string` | SecretRef URI for the new passphrase. | - | packages/cli/src/commands/storage.ts:134 |
...
| <a id="property-swap"></a> `swap?` | `readonly` | `boolean` | If `true`, atomically swap the encrypted target into the `storage.path` location after the integrity check, leaving the original under `<storage.path>.bak.<timestamp>`. Default `false`. | - | packages/cli/src/commands/storage.ts:145 |
| <a id="property-targetpath"></a> `targetPath?` | `readonly` | `string` | Optional explicit target path for the encrypted output. Default: `<storage.path>.encrypted`. | - | packages/cli/src/commands/storage.ts:139 |
