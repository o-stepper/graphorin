...
| ------ | ------ | ------ | ------ | ------ |
| <a id="property-calltimeoutms"></a> `callTimeoutMs?` | `readonly` | `number` | Per-call timeout (ms) applied to every adapted tool's `client.callTool` invocation (MC-3/MC-5). Default: the SDK default. | packages/mcp/src/client/types.ts:215 |
