...
| <a id="property-channels"></a> `channels` | `readonly` | \| [`ChannelsDaemon`](/api/@graphorin/server/interfaces/ChannelsDaemon.md) \| `undefined` | B1.6: optional channels daemon - populated when the operator wired a channel gateway via `createServer({ channels })`. | packages/server/src/app.ts:106 |
...
| <a id="property-workflowtimers"></a> `workflowTimers` | `readonly` | \| [`WorkflowTimerDaemon`](/api/@graphorin/server/interfaces/WorkflowTimerDaemon.md) \| `undefined` | W-032: optional workflow durable-timer daemon - populated when the operator wired a `createTimerDriver(...)` at construction time. | packages/server/src/app.ts:101 |
