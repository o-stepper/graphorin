...
| <a id="property-verifier"></a> `verifier?` | `readonly` | \{ `invalidate`: `void`; \} | The live token verifier, so a REST revoke invalidates its LRU entry immediately - otherwise a just-used token keeps authenticating from the cache for up to `cacheTtlMaxMs` (60s). | packages/server/src/routes/tokens.ts:32 |
| `verifier.invalidate` | `public` | `void` | - | packages/server/src/routes/tokens.ts:32 |
