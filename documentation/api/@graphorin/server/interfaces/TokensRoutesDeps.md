...
| <a id="property-verifier"></a> `verifier?` | `readonly` | \{ `invalidate`: `void`; \} | TOKENS-RE-01 / SPL-9: the live token verifier, so a REST revoke invalidates its LRU entry immediately - otherwise a just-used token keeps authenticating from the cache for up to `cacheTtlMaxMs` (60s). | packages/server/src/routes/tokens.ts:33 |
