...
- the **`@pydantic/genai-prices` dataset** itself: the published `data.json` from that repository is auto-detected and converted (per-Mtok figures divide into per-token USD, cache read/write legs included). A working example against the upstream dataset:
