 ...
 *                     section to the two larger artefacts. W-129: the
 *                     ~2800 generated API pages used to live here too,
 *                     inflating the "short index" to ~493 KB (~120k
 *                     tokens); they now live in llms-api.txt.
...
export function buildArtifacts({ index, api, full }) {
  const llmsTxt = [
    ...
    '## Optional',
    '',
    `- [API reference index](${SITE_URL}/llms-api.txt): one line per generated API page (~2800 entries)`,
    `- [Full documentation snapshot](${SITE_URL}/llms-full.txt): concatenated Markdown of every published page`,
    '',
  ].join('\n');

  const llmsApiTxt = [
    '# Graphorin - API reference index',
    '',
    '> Index of the generated API reference pages (TypeDoc output for every',
    '> @graphorin/* package). The short site index lives at /llms.txt.',
    '',
...
async function main() {
  ...
  if (indexBytes > MAX_INDEX_BYTES) {
    console.error(
      `[graphorin/docs] llms.txt is ${indexBytes} bytes (> ${MAX_INDEX_BYTES}): ` +
  ...
  console.log(
    `[graphorin/docs] llms.txt: wrote ${indexBytes} chars to public/llms.txt (ceiling ${MAX_INDEX_BYTES})`,
  );
  console.log(
    `[graphorin/docs] llms-api.txt: wrote ${llmsApiTxt.length} chars to public/llms-api.txt`,
  );
  console.log(
  ...
  try {
    ...
    console.log('[graphorin/docs] llms artefacts mirrored into .vitepress/dist');
  } catch {
...
function selfTest() {
  ...
  const fixture = {
    ...
    api: [
      {
        label: 'API reference',
        pages: [page(`${SITE_URL}/api/@graphorin/core/functions/createAgent`, 'createAgent')],
      },
    ],
    full: [],
  };
  ...
  const cases = [
    ['llms.txt has no /api/ page links', !llmsTxt.includes('/api/@graphorin')],
    ...
    ['llms-api.txt carries the api pages', llmsApiTxt.includes('/api/@graphorin/core/functions')],
