...
- **`llm/`** - an LLM-as-judge scorer (`llmJudge`) for open-ended answers. Hardened against prompt injection in the candidate output. A reply with no `SCORE: <n>` marker earns ONE constrained re-ask with a raised output budget (reasoning-model judges can burn a tight `maxOutputTokens` on hidden reasoning and return an empty reply; `offFormatRetries: 0` restores single-shot); if the retry also fails, the scorer throws the typed `JudgeOffFormatError` - a judge failure distinguishable from a genuine low grade. Prefer a stronger model as the judge than as the subject for strict QA scoring.
