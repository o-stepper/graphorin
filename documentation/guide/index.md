...
Graphorin is currently on the `v0.15.1` pre-release line. The framework follows lockstep versioning across all `@graphorin/*` packages while on the `0.x` line; once it reaches `1.0`, optional packages and adapters are versioned independently.
