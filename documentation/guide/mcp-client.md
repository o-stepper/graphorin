...
- defaults each generated tool to `sideEffectClass: 'external-stateful'` (operators downgrade per-tool through `sideEffectClassByTool`). A downgrade to `'read-only'` / `'pure'` is a wide trust decision: sink classification is fully metadata-driven, so the tool leaves **every** sink check at once - the data-flow gate, the Rule-of-Two writer forbid, and the read-only capability gate. The server's own `readOnlyHint` is deliberately **not** trusted for this (a hostile server could self-declare read-only). Downgrade only tools whose read-only nature you have verified yourself; each downgrade logs one WARN at adaptation time (the auditable record - `toTools()` returns a plain `Tool[]`, there is no separate downgrade-list return value);
