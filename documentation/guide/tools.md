...
| `memoryGuardTier` | `MemoryGuardTier` (DEC-153) | Classification for the pre/post memory-snapshot guard. **Active when the agent has `memory` wired**; skipped with a one-time WARN otherwise - see [Memory-modification guard](#memory-modification-guard). |
...
Known limitation: the projection covers **eager** tools only (progressive disclosure - deferred matches fold in through `code_search`). Under the [minimal scaffold](/guide/minimal-profile#the-scaffold-minimal-preset), where every undeclared tool defers by default, the initial projection is therefore near-empty; prefer direct tool calls over code-mode for minimal-scaffold agents.
...
**The runtime is a seam.** `runBridgedSource` is only the DEFAULT implementation of the `CodeModeRunner` contract (`@graphorin/security/sandbox`): `(options: BridgedSourceOptions) => Promise<BridgedSourceResult>`. `AgentConfig.codeMode.run` substitutes where the script executes - a subprocess provider, a remote runner - and `codeMode.limits` bounds it (`timeoutMs`, `maxMemoryMb`, `maxToolCalls`) whatever the runtime; both thread through `createCodeExecuteTool({ run, limits })`. The provider contract is fixed: the runner receives ONLY the script source, the allowed tool names, the host `dispatch` bridge, the cancellation signal and the limits - credentials, `RunState` and policy never cross the seam, because every in-script `tools.<name>(args)` call routes back through `dispatch` into the host's tool executor where the full governance stack applies. A provider that needs more than that is out of contract.

## Execution limits

Inline tools (the `tool({...})` closures the agent runs in-process) are bounded by an **enforced wall-clock timeout**: the tier-resolved per-tool `timeoutMs` when set, else `createToolExecutor({ inlineToolTimeoutMs })` (default 60 s; an explicit executor option wins over tier defaults). Expiry fails the call with `ToolError({ kind: 'timeout' })` and the run continues; a tool that hangs and ignores `ctx.signal` can no longer block a run indefinitely. Sandbox tiers keep their own per-tier timeouts, and a sandbox's structured failure maps onto the honest kind: a sandbox `timeout` surfaces as `'timeout'`, a `sandbox-violation` or `memory-exceeded` as `'sandbox_violation'`. Tools whose upstream service rate-limits them can throw `ToolRateLimitError` (from `@graphorin/tools`) to surface `'rate_limited'` with a `retry after Nms` hint instead of a generic `execution_failed`.

## Memory-modification guard

Every tool declares a `memoryGuardTier` - one of `'pure'`, `'side-effecting-no-memory'`, `'memory-aware'`, `'unknown'`, or `'untrusted'`. When a memory-region reader is supplied, the executor snapshots the affected region before a memory-aware call, verifies it after, and audits any unexpected drift (`memory:modification:before` / `memory:modification:after`). **With `memory` wired on the agent this step is active**: the runtime binds a scope-aware region reader over the working-memory tier (the scope resolves from the in-flight run), so the snapshot/verify cycle runs for every guarded tier. Without `memory`, the guard is skipped and the agent emits a one-time WARN when any tool declares a `memoryGuardTier` - the silent no-op is visible.
...
|---|---|---|
| `'none'` | The Node.js process. | Trusted, in-process tools. Default for trusted built-ins (`'built-in'`); user-defined inline tools default to `worker-threads`. |
