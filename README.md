...
Latest release: **0.13.1** (2026-07-19) - the honest-docs patch, closing the third external deep retest: GPT-5.6 cache writes are now billed at the official 1.25x premium (estimates were 20% low on that leg) and bare `gpt-5.6` is priced as the sol alias it is; the quickstart actually performs its persist-and-survive-restart promise, with a consumer smoke proving the flow against the published packages; the public API reference dropped every internal audit label (522 to 0 across 3187 pages, now gated in CI); and the docs site ships a 9x smaller client search index. Under the hood the workspace moved to vitest 4, fixing coverage collapsing in checkout paths that contain a space. See the [changelog](./CHANGELOG.md) and the [migration guide](https://graphorin.com/guide/migration#_0-13-0-0-13-1) for upgrade notes.
