...
[![Version: 0.12.1](https://img.shields.io/badge/version-v0.12.1-blue.svg)](./CHANGELOG.md)
...
Graphorin is currently on the **`v0.12.1`** pre-release line, **published on the npm registry** under the `@graphorin/*` scope. The framework follows **lockstep versioning** across all `@graphorin/*` packages while the framework is on the `0.x` line; once it reaches `1.0`, optional packages and adapters are versioned independently.
...
Latest release: **0.12.1** (2026-07-17) - the live-verification patch: the first billed real-provider pass over the published packages found (and this release fixes) one defect - the Anthropic OpenAI-compat endpoint rejects every permissive `response_format`, so schema-less structured requests now send none and ride the agent's JSON instruction + local parse gate instead. The repository also gains enforced CI coverage thresholds, the first real-provider LongMemEval baselines (338/500 with a haiku subject and a local judge), the `benchmarks/conformance` framework-floor suite, and two new examples (`assistant-bot`, the official whole-bot recipe, and `structured-verifier`). See the [changelog](./CHANGELOG.md) and the [migration guide](https://graphorin.com/guide/migration#_0-12-0-0-12-1) for upgrade notes.
...
**Graphorin** · v0.12.1 · MIT License · © 2026 Oleksiy Stepurenko
