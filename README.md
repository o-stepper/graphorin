...
[![Version: 0.13.1](https://img.shields.io/badge/version-v0.13.1-blue.svg)](./CHANGELOG.md)
...
Graphorin is currently on the **`v0.13.1`** pre-release line, **published on the npm registry** under the `@graphorin/*` scope. The framework follows **lockstep versioning** across all `@graphorin/*` packages while the framework is on the `0.x` line; once it reaches `1.0`, optional packages and adapters are versioned independently.
...
Latest release: **0.13.1** (2026-07-19) - the fail-closed budget release, closing every finding of the second external deep retest: an unpriced `RunBudget.maxCostUsd` now **stops the run** instead of warning and spending unmetered (`RunBudget.onUnpriced`, default `'fail'`); the bundled pricing snapshot knows the current GPT-5.6 line (luna/terra/sol) so cost caps observe real spend there; `graphorin init --cloud-consent` hands you the exact `createMemory` privacy snippet that enforces the chosen tier; and `doctor --all` no longer false-fails on a deliberately-disabled audit log. The repository also retires the spaced-path silent no-op class (14 entrypoints fixed + a new CI gate with a spaced-directory proof) and cuts TypeDoc warnings from 2721 to ~115. See the [changelog](./CHANGELOG.md) and the [migration guide](https://graphorin.com/guide/migration#_0-12-1-0-13-0) for upgrade notes.
...
**Graphorin** · v0.13.1 · MIT License · © 2026 Oleksiy Stepurenko
