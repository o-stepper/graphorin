...
Latest release: **0.15.1** (2026-07-22) - a one-fix patch on top of the proof minor 0.15.0, caught by the release's own published-surface smoke minutes after publish: `createRequestTimeout` (behind the new opt-in `timeoutMs` on the Vercel AI SDK and in-process GGUF adapters) `unref`'d its deadline timer, so a bare consumer script whose transport held no event-loop handle could exit mid-call instead of observing the honest timeout error - the timer now keeps the loop alive, matching the HTTP adapters, with a subprocess regression test pinning the semantics. The 0.15.0 story (golden paths, unified provider recovery semantics, CI-earned operational claims incl. the crash-resume drill and soak SLOs, `409 agent-busy`, benchmark evidence) is one section down in the [changelog](./CHANGELOG.md); upgrade notes in the [migration guide](https://docs.graphorin.com/guide/migration#_0-15-0-0-15-1).
